...
const TaskCard = forwardRef<HTMLDivElement, ComponetProps>(({ task }) => {
  const clippedText =
  ...
  return (
    <div className="w-full shadow-shadow-primary-xsm  py-2 pb-0 pt-4 relative rounded-lg flex flex-col">
      <div className="px-3">
        <span
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <span className="text-text-secondary text-sm">{clippedText}</span>
      ...
      <div className="py-2 px-3 mt-4 border-t text-text-secondary border-border-primary flex justify-between items-center">
        ...
        <span className="text-xs">
          Due date {dayjs(task.$createdAt).format("D MMMM YYYY")}
        </span>
      </div>
    </div>
  );
